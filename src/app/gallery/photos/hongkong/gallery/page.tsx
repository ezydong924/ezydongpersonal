...
const INITIAL_SCALE_FACTOR = 2.8; // Start zoomed in: fewer images visible initially
...
export default function HongKongGallery() {
  ...
  const layout = useMemo(() => buildLayout(PHOTOS.length), []);
  ...
  const measure = useCallback(() => {
    ...
    maxScale.current = mx;
    if (!ready) {
      scale.current = mn * INITIAL_SCALE_FACTOR;
      scaleTarget.current = mn * INITIAL_SCALE_FACTOR;
      cam.current = { x: 0, y: 0 };
      setReady(true);
    } else {
  ...
  const resetView = () => {
    scaleTarget.current = minScale.current * INITIAL_SCALE_FACTOR;
    cam.current = { x: 0, y: 0 };
    vel.current = { x: 0, y: 0 };
  };
  ...
  return (
    ...
    >
      <style>{`
        .hk-gallery { cursor: grab; }
        .hk-gallery:active { cursor: grabbing; }
        .hk-tile { transition: filter .35s ease, box-shadow .35s ease; cursor: pointer; }
        .hk-tile:hover { filter: brightness(1.14) saturate(1.05); box-shadow: 0 24px 60px rgba(0,0,0,.6) !important; z-index: 50; }
        .hk-zbtn:hover { background: rgba(255,255,255,.14) !important; }
        @keyframes hkLbIn { from { opacity:0; } to { opacity:1; } }
        @keyframes hkLbPop { from { opacity:0; transform: scale(.92); } to { opacity:1; transform: scale(1); } }
      `}</style>

      <div className="fixed top-8 left-8 z-50" style={{ zIndex: 60 }}>
        <BackButton href="/gallery/photos/hongkong" label="返回" />
      </div>
      ...
      <div style={{ position: "absolute", top: "50%", left: "50%", width: 0, height: 0 }}>
        {ready && layout.map((it, k) => (
          ...
          >
            <img
              src={`/hongkong/thumbs/${PHOTOS[it.i]}`}
              alt=""
              draggable={false}
              decoding="async"
      ...
      <div style={{ position: "absolute", top: 36, left: 120, pointerEvents: "none", zIndex: 10 }}>
        ...
        <div style={{ fontSize: 30, color: "#f2efe9", marginTop: 6, fontWeight: 500 }}>香港</div>
      </div>
      ...
      <div style={{ position: "absolute", right: 36, bottom: 72, zIndex: 60, display: "flex", flexDirection: "column", gap: 8 }}
        ...
        <div className="hk-zbtn" style={btnStyle} onClick={() => zoomBtn(1)}><Plus size={17} /></div>
        <div className="hk-zbtn" style={btnStyle} onClick={() => zoomBtn(-1)}><Minus size={17} /></div>
        <div className="hk-zbtn" style={btnStyle} onClick={resetView} title="全景"><Maximize2 size={15} /></div>
        <div className="hk-zbtn" style={btnStyle} onClick={() => musicStore.togglePlay()} title={musicPlaying ? "暂停" : "播放"}>
      ...
      {active !== null && (
        <div onClick={() => setActive(null)} style={{
          ...
          cursor: "zoom-out", animation: "hkLbIn .3s ease" }}>
          <div onClick={(e) => e.stopPropagation()} style={{
            maxWidth: "82vw", maxHeight: "76vh", borderRadius: 6, overflow: "hidden",
            boxShadow: "0 40px 120px rgba(0,0,0,.7)", animation: "hkLbPop .35s cubic-bezier(.2,.8,.2,1)", cursor: "default" }}>
            <img src={`/hongkong/lightbox/${activePhoto}`} alt=""
              decoding="async"
