...
const categories = [
  { name: "晨光", icon: "🌅", slug: "dawn", desc: "梅里日照金山" },
  { name: "午后", icon: "☀️", slug: "noon", desc: "纳帕海草甸" },
  { name: "蓝调", icon: "🌆", slug: "blue", desc: "独克宗古城暮色" },
  { name: "夜景", icon: "🌃", slug: "night", desc: "松赞林寺星光" },
];
        ...
        <div className="grid grid-cols-2 gap-4 w-full max-w-lg">
          {categories.map((cat, i) => (
            <motion.div key={cat.slug} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
              <Link href={"/gallery/photos/diqing/" + cat.slug} className="block p-6 rounded-2xl bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.08] hover:border-white/15 transition-all duration-300 text-center">
