...
const INITIAL_SCALE_FACTOR = 2.8;
...
export default function XishuangbannaGallery() {
  ...
  return (
    ...
    >
      <style>{`
        .xn-gallery { cursor: grab; }
        .xn-gallery:active { cursor: grabbing; }
        .xn-tile { transition: filter .35s ease, box-shadow .35s ease; cursor: pointer; }
        .xn-tile:hover { filter: brightness(1.14) saturate(1.05); box-shadow: 0 24px 60px rgba(0,0,0,.6) !important; z-index: 50; }
        .xn-zbtn:hover { background: rgba(255,255,255,.14) !important; }
        @keyframes xnLbIn { from { opacity:0; } to { opacity:1; } }
        @keyframes xnLbPop { from { opacity:0; transform: scale(.92); } to { opacity:1; transform: scale(1); } }
      `}</style>
      ...
      <div style={{ position: "absolute", top: "50%", left: "50%", width: 0, height: 0 }}>
        {ready && layout.map((it, k) => (
          ...
          >
            <img
              src={`/xishuangbanna/thumbs/${PHOTOS[it.i]}`}
              alt=""
              draggable={false}
              decoding="async"
      ...
      <div style={{ position: "absolute", top: 36, left: 120, pointerEvents: "none", zIndex: 10 }}>
        ...
        <div style={{ fontSize: 30, color: "#f2efe9", marginTop: 6, fontWeight: 500 }}>西双版纳</div>
      </div>
      ...
      <div style={{ position: "absolute", right: 36, bottom: 72, zIndex: 60, display: "flex", flexDirection: "column", gap: 8 }}
        ...
        <div className="xn-zbtn" style={btnStyle} onClick={() => zoomBtn(1)}><Plus size={17} /></div>
        <div className="xn-zbtn" style={btnStyle} onClick={() => zoomBtn(-1)}><Minus size={17} /></div>
        <div className="xn-zbtn" style={btnStyle} onClick={resetView} title="全景"><Maximize2 size={15} /></div>
        <div className="xn-zbtn" style={btnStyle} onClick={() => musicStore.togglePlay()} title={musicPlaying ? "暂停" : "播放"}>
      ...
      {active !== null && (
        <div onClick={() => setActive(null)} style={{
          ...
          cursor: "zoom-out", animation: "xnLbIn .3s ease" }}>
          <div onClick={(e) => e.stopPropagation()} style={{
            maxWidth: "82vw", maxHeight: "76vh", borderRadius: 6, overflow: "hidden",
            boxShadow: "0 40px 120px rgba(0,0,0,.7)", animation: "xnLbPop .35s cubic-bezier(.2,.8,.2,1)", cursor: "default" }}>
            <img src={`/xishuangbanna/lightbox/${activePhoto}`} alt=""
              decoding="async"
